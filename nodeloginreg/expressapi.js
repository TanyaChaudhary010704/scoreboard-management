...
app.post("/studentlogin", async (req, res) => {
  try {
    ...
    const status = arr.find((ele) => ele.roll == roll && ele.dob == dob);
    if (status) {
...
app.post("/adminlogin", async (req, res) => {
  try {
    ...
    const data = await fs.readFile("adminData.json", { encoding: "utf-8" });
    arr = JSON.parse(data);

    const status = arr.find((ele) => ele.id == id && ele.password == password);
    if (status) {
      return res.json({ message: "Successfully Login" });
    } else {
